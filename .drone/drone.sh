...
['mkdir -p $TRAVIS_BUILD_DIR/coverals', 'find ../../../bin.v2/ -name "*.gcda" -exec cp "{}" $TRAVIS_BUILD_DIR/coverals/ \\;', 'find ../../../bin.v2/ -name "*.gcno" -exec cp "{}" $TRAVIS_BUILD_DIR/coverals/ \\;', 'git clone https://github.com/linux-test-project/lcov.git lcov_dir', 'GCOV_VERSION=""', 'if [[ "$TOOLSET" == *"-"* ]]; then GCOV_VERSION="--gcov-tool gcov-${TOOLSET#*-}"; fi', 'LCOV="$BOOST/libs/$PROJECT_TO_TEST/test/lcov_dir/bin/lcov $GCOV_VERSION"', '$LCOV --directory $TRAVIS_BUILD_DIR/coverals --base-directory ./ --capture --output-file $TRAVIS_BUILD_DIR/coverals/coverage.info', 'cd $BOOST', '$LCOV --remove $TRAVIS_BUILD_DIR/coverals/coverage.info "*.cpp" "/usr*" "*/$PROJECT_TO_TEST/test/*" $IGNORE_COVERAGE "*/$PROJECT_TO_TEST/tests/*" "*/$PROJECT_TO_TEST/examples/*" "*/$PROJECT_TO_TEST/example/*" -o $TRAVIS_BUILD_DIR/coverals/coverage.info', 'OTHER_LIBS=`grep "submodule .*" .gitmodules | sed \'s/\\[submodule\\ "\\(.*\\)"\\]/"\\*\\/boost\\/\\1\\.hpp" "\\*\\/boost\\/\\1\\/\\*"/g\'| sed "/\\"\\*\\/boost\\/process\\/\\*\\"/d" | sed \':a;N;$!ba;s/\\n/ /g\'`', 'echo $OTHER_LIBS', 'eval "$LCOV --remove $TRAVIS_BUILD_DIR/coverals/coverage.info $OTHER_LIBS -o $TRAVIS_BUILD_DIR/coverals/coverage.info" > /dev/null', 'cd $TRAVIS_BUILD_DIR', 'gem install coveralls-lcov', 'coveralls-lcov coverals/coverage.info']
